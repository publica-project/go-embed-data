...
func GetStatic(file, key, nonce string) ([]byte, error) {
	...
	ct, err := hex.DecodeString(string({{.Var}}[file]))
	if err != nil {
		return nil, err
	}
	return aesgcm.Open(nil, xor([]byte(nonce), []byte(file)), ct, nil)
}
{{end}}
`))
...
func (c *configuration) Key(filename string) string {
	return strconv.Quote(c.nonceKey(filename))
}

func (c *configuration) nonceKey(filename string) string {
...
func newConfiguration(args []string) (*configuration, error) {
	...
	flags.StringVar(&c.EncryptSharedKey, "encrypt-shared-key", "", "AES-GCM key")
	flags.StringVar(&c.encryptNonce, "encrypt-nonce", "", "AES-GCM nonce")
...
func embedSingleFile(c *configuration, w io.Writer) error {
	...
	if err := embedFileContents(w, c, c.sources[0]); err != nil {
		return err
	}
...
func findSources(c *configuration, cb func(i int, f io.Reader) error) error {
	...
	for _, filename := range c.sources {
		fmt.Printf("file: %s\n", c.nonceKey(filename))
		if c.EncryptSharedKey != "" {
			r, err := encryptedReader(c, filename)
			if err != nil {
				return err
			}
			err = cb(len(c.FoundSources), r)
			if err != nil {
				return err
			}
		} else {
...
func encryptedReader(c *configuration, filename string) (io.Reader, error) {
	b, err := aes.NewCipher([]byte(c.EncryptSharedKey))
	if err != nil {
		return nil, err
	}
	aesgcm, err := cipher.NewGCM(b)
	...
	return strings.NewReader(hex.EncodeToString(aesgcm.Seal(nil, xor([]byte(c.encryptNonce), []byte(c.nonceKey(filename))), plaintext, nil))), nil
}

func embedFileContents(w io.Writer, c *configuration, filename string) error {
	fmt.Printf("file: %s\n", c.nonceKey(filename))
	if c.EncryptSharedKey != "" {
		r, err := encryptedReader(c, filename)
		if err != nil {
		...
		_, err = io.Copy(&escapeWriter{w}, r)
		return err
	}
